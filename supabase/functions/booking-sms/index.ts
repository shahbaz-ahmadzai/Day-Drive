...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      await db.from("notifications").insert({ channel: "sms", provider: "twilio", template, recipient: to, body: text, booking_id: bookingId, status: "sent", provider_message_id: sent.sid, created_by: userId });
      ...
      await db.from("notifications").insert({ channel: "sms", provider: "twilio", template, recipient: to, body: text, booking_id: bookingId, status: "failed", error: msg, created_by: userId });
